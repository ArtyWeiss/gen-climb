...
public class OrbitCamera : MonoBehaviour
{
    ...
    private Vector3 CameraHalfExtends
    {
        ...
        {
            Vector3 halfExtends;
            halfExtends.y = cameraData.camera.nearClipPlane *
                            Mathf.Tan(0.5f * Mathf.Deg2Rad * cameraData.camera.fieldOfView);
    ...
    private bool ManualRotation()
    {
        ...
        if (input.x >= -Mathf.Epsilon && input.x <= Mathf.Epsilon && input.y >= -Mathf.Epsilon &&
            input.y <= Mathf.Epsilon) return false;
    ...
    private bool AutomaticRotation()
    {
        ...
        var movement = new Vector2(focusPosition.x - previousFocusPosition.x,
    ...
    {
        cameraData.orbitAngles.x = Mathf.Clamp(cameraData.orbitAngles.x, cameraData.minVerticalAngle,
    ...
    private void SetCameraFov()
    {
        ...
        var relativeSpeed = Mathf.Clamp01(movementData.body.velocity.magnitude /
                                          Mathf.Max(movementData.maxSprintSpeed, movementData.maxSpeed));
        var targetViewAngle =
            Mathf.Lerp(cameraData.defaultCameraViewAngle, cameraData.maxCameraViewAngle, relativeSpeed);
    ...
    private void CheckCameraObstructions(Vector3 lookDirection, Quaternion lookRotation, ref Vector3 lookPosition)
    {
        ...
        if (!Physics.BoxCast(castSource, CameraHalfExtends, castDirection, out RaycastHit hit, lookRotation,
            castDistance, cameraData.obstructionMask)) return;
